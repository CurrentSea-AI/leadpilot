...
export default function LeadsPage() {
  ...
  const downloadReport = async (lead: Lead) => {
    // Get report data
    const reportData = {
      lead: { name: lead.name, websiteUrl: lead.websiteUrl, city: lead.city },
      designScore: lead.designAudit?.score || (lead.audit?.score || 0) * 10,
      seoScore: lead.seoAudit?.score,
      findings: lead.designAudit?.findingsJson || lead.audit?.findingsJson || "[]",
    };
    
    // Create a simple text-based report for now
    const findings = JSON.parse(reportData.findings);
    const reportText = `
═══════════════════════════════════════════════════════════
                    WEBSITE AUDIT REPORT
═══════════════════════════════════════════════════════════

Business: ${reportData.lead.name}
Website: ${reportData.lead.websiteUrl}
${reportData.lead.city ? `Location: ${reportData.lead.city}` : ""}

═══════════════════════════════════════════════════════════
                         SCORES
═══════════════════════════════════════════════════════════

Design Score: ${reportData.designScore}/100
${reportData.seoScore ? `SEO Score: ${reportData.seoScore}/100` : ""}

═══════════════════════════════════════════════════════════
                        FINDINGS
═══════════════════════════════════════════════════════════

${findings.map((f: { issue?: string; recommendation?: string } | string, i: number) => {
  if (typeof f === "string") return `${i + 1}. ${f}`;
  return `${i + 1}. ${f.issue || f}\n   → ${f.recommendation || ""}`;
}).join("\n\n")}

═══════════════════════════════════════════════════════════
                     GENERATED BY LEADPILOT
═══════════════════════════════════════════════════════════
`;
